...
import { Categorie, Fournisseur, Produit, Stock } from "@prisma/client";
import {
  InsertBonCommande,
  obtenirFournisseurs,
  searchFournisseurs,
...
const NewBonCommandForm = () => {
  ...
  const [clients, setClients] = useState<Fournisseur[] | undefined>([]);
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const clients = await obtenirFournisseurs(5);
        setClients(clients);
  ...
  useEffect(() => {
    const searchForClients = async () => {
      try {
        const clients = await searchFournisseurs(searchQuery);
        setFilteredClients(clients);
      } catch (error) {
  ...
  const handelSubmit = async () => {
    try {
      ...
      await InsertBonCommande(clientId, date, rows);
      alert("inserted");
    } catch (error) {
                ...
                <SelectContent>
                  ...
                  {filteredClients?.map((client) => (
                    <SelectItem
                      ...
                      key={client.fournisseur_id}
                      value={client.fournisseur_id.toString()}
                    >
                      <p>{client.nom}</p>
                      <p>{client.email}</p>
                    </SelectItem>
...
export default NewBonCommandForm;
