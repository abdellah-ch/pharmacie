...
export async function GET(req: NextRequest) {
  ...
  const pdfBytes = await generateColisPdf(colis);
  return new NextResponse(pdfBytes, {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="colis-${commandeId}.pdf"`,
    },
  });
}

// Function to generate Colis PDF
async function generateColisPdf(colis: any): Promise<Uint8Array> {
